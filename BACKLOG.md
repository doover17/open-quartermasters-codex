...
The master reference list for the v0.1 release: **100 items** (60 gear, 40 consumables). Each row is one file to be written per `docs/item-template.md`, `docs/style-guide.md`, and `docs/pricing-guide.md`.

**Status key:** ✅ merged · 🔨 claimed/in progress · ⬜ open

Each entry lists the intended `id`, quality tier, and a one-line design intent — the seed the writer grows the item from. Intent lines are suggestions, not contracts; writers may take an item a better direction if the id and tier stay stable.

---

## Gear (60 items)

### Batch 1 — Core Kit ✅ (10/10 merged)

| Status | id | Tier | Design intent |
| --- | --- | --- | --- |
| ✅ | fine-bedroll | fine | Exhaustion protection; the visible kindness of lending it |
| ✅ | shoddy-rope | shoddy | The cheap coil that snaps at the worst moment |
| ✅ | masterwork-crowbar | masterwork | Leverage that never fails; a smith who vets buyers |
| ✅ | waxed-map-case | common | Why the treasure map survives the river |
| ✅ | bone-signal-whistle | common | Split-party communication for 5 sp |
| ✅ | fine-tinderbox | fine | Fire in wind and rain |
| ✅ | trail-rations | common | The wilderness countdown clock, with spoilage |
| ✅ | shoddy-lantern | shoddy | Double oil burn, leaky hood |
| ✅ | chalk-bundle | common | Maze-crawler's insurance |
| ✅ | fine-waterskin | fine | Water that doesn't spoil in the desert arc |

### Batch 2 — Dungeoneering ✅ (10/10 merged)

| Status | id | Tier | Design intent |
| --- | --- | --- | --- |
| ✅ | fine-grappling-hook | fine | Silent tines for night work |
| ✅ | iron-piton-set | common | Security bought with noise |
| ✅ | folding-ten-foot-pole | fine | Trap-probing as a declared playstyle |
| ✅ | oil-flask | common | Fuel or weapon, never both |
| ✅ | sputter-torches | shoddy | 30-minute light with a gutter-out die |
| ✅ | steel-inspection-mirror | common | Corners checked with a wrist, not a head |
| ✅ | oak-door-wedges | common | "Run!" becomes a fighting retreat |
| ✅ | fine-lockpicks | fine | The product is "no tool marks" |
...
## Brief for Writers & Agents

Any writer — human or agent — filling backlog slots follows this exact procedure:

1. **Read three docs first:** `docs/item-template.md`, `docs/style-guide.md`, `docs/pricing-guide.md`. They are the specification; this backlog is only the shopping list.
2. **One item per file** at `items/<category>/<id>.md`. The `id` in this backlog is final — filename and front matter must match it.
3. **Follow the design intent** loosely: it defines the item's reason to exist, not its final text. If your version is better, keep the id and tier, change the rest.
4. **Complete YAML front matter** — every field, both system blocks. Flag `TODO:` only if genuinely unable to convert.
